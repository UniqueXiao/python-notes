...
#生成元组 tuple是一个 有序 序列，其中的元素用 , 隔开，元组元素不需要是同一类型,甚至可以包含元组,但是元组是 不可变的（Immutable）
tup_1 = ()
tup_2 = tuple()

#索引和切片
tup_3 = (1,2,3,4,5)
print tup_3[1]   #输出2，索引默认从0开始
print tup_3[1:4] #输出(2,3,4)
#元组方法  由于元组是不可变的，因此只有一些不改变元组的方法
print tup_3.count(2)  #输出1，与列表类似，计算元素出现的次数
print tup_3.index(5)  #输出4，与列表类似，返回元素第一次出现的索引位置
#列表与元组的转换
lst_test_1 = list(tup_3)  #将元组转换为列表
lst_test_2 = [9,8,7,6,5]
tup_4 = tuple(lst_test)   #将列表转换为元组
print lst_test_1          #输出[1,2,3,4,5]
print tup_4               #输出(9,8,7,6,5)
#为什么需要元组
  '''旧式字符串格式化中参数要用元组；
     在字典中当作键值；
     数据库的返回值…… '''
